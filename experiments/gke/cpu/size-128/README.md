...
Note that this cluster took days of trying, and I got it early afternoon on a Sunday.
...
**Did not work (also didn't work at 64) - MPI errors**
...
helm install \
  ...
  --set amg.processor_topology="32 32 11" \
...
**Did not work for much smaller sizes**
...
helm install \
  --set experiment.nodes=128 \
  --set minicluster.size=128 \
  --set minicluster.tasks=5376 \
  --set experiment.tasks=5376 \
  --set minicluster.save_logs=true \
  --set quicksilver.inputfile="/opt/quicksilver/Examples/CORAL2_Benchmark/Problem1/Coral2_P1.inp" \
  --set quicksilver.X=256 \
  --set quicksilver.Y=128 \
  --set quicksilver.Z=128 \
  --set quicksilver.x=256 \
  --set quicksilver.y=128 \
  --set quicksilver.z=128 \
  --set quicksilver.I=32 \
  --set quicksilver.J=14 \
  --set quicksilver.K=12 \
  --set env.OMP_NUM_THREADS=2 \
  --set quicksilver.n=1342117280 \
  --set experiment.iterations=5 \
  qs ./quicksilver
