...
def main():
    ...
    files = ps.find_inputs(indir, "remhos")
    if not files:
...
def parse_data(indir, outdir, files):
    ...
    print("Done parsing remhos results!")

    # Save stuff to file first
    p.df.to_csv(os.path.join(outdir, "remhos-results.csv"))
...
def plot_results(df, outdir, non_anon=False):
    ...
    for metric, data_frames in frames.items():
        ...
        title = " ".join([x.capitalize() for x in metric.split('_')])
        axes[0].set_title(f"Remhos {title} (CPU)", fontsize=14)
        if "duration" == metric:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(img_outdir, f"remhos-{metric}-cpu.svg"))
        plt.savefig(os.path.join(img_outdir, f"remhos-{metric}-cpu.png"))
        plt.clf()
