...
def main():
    ...
    files = ps.find_inputs(indir, "pennant")
    if not files:
...
def parse_data(indir, outdir, files):
    ...
    for filename in files:
        ...
        for job, metadata in jobs.items():
            ...
            # The FOM for PENNANT benchmarks is total zones processed per second
            # which can be computed from three lines in PENNANT standard output
            # PENNANT standard output:
            lines = metadata["log"].split("\n")
            # • number of zones in mesh: in the line ‘Zones:’ near the top
            zones = int([x for x in lines if "Zones" in x][0].split()[-1])
            # • number of cycles run: in line ‘cycle =’ near the bottom
            cycles = int([x for x in lines if "cycle =" in x][0].split()[2].strip(","))
            # • run time: in line ‘hydro cycle run time=’ near the bottom
            # (note that this time excludes initialization and finalization stages)
            run_time = float(
                [x for x in lines if "hydro cycle run time=" in x][0].split()[-1]
            )
            # FOM = zones * cycles / time
            fom = zones * cycles / run_time
            p.add_result("fom", fom)

    print("Done parsing pennant results!")

    # Save stuff to file first
    p.df.to_csv(os.path.join(outdir, "pennant-results.csv"))
...
def plot_results(df, outdir, non_anon=False):
    ...
    for metric, data_frames in frames.items():
        ...
        if "duration" == metric:
            ...
            axes[0].set_title(f"Pennant {title} (CPU)", fontsize=14)
        else:
            axes[0].set_ylabel("Zones processed / Second", fontsize=14)
            axes[0].set_title(f"Pennant Zones Processed per Second (CPU)", fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(img_outdir, f"pennant-{metric}-cpu.svg"))
        plt.savefig(os.path.join(img_outdir, f"pennant-{metric}-cpu.png"))
        plt.clf()
