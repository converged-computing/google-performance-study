...
def parse_data(indir, outdir, files):
    ...
    for filename in files:
        ...
        # These are associated yaml files
        result_foms = list(ps.get_outfiles(os.path.dirname(filename), pattern="[.]yaml"))

        problem_size = "230nx-ny-nz"
        for i, job in enumerate(jobs):
            metadata = jobs[job]
            p.add_result("duration", metadata['duration'], problem_size)
            data_item = ps.read_yaml(result_foms[i])

            # Just parse smaller problem size
            dims = data_item["Global Run Parameters"]["dimensions"]
            if dims["nx"] != 230:
                continue

            fom = data_item["CG solve"]["Total"]["Total CG Mflops"]
...
def plot_results(df, outdir, non_anon=False):
    ...
    #            "google/compute-engine/cpu",
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(img_outdir, f"minife-{metric}-cpu.svg"))
        plt.savefig(os.path.join(img_outdir, f"minife-{metric}-cpu.png"))
        plt.clf()
